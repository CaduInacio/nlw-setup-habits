...
export function HabitDay({ amount = 0, defaultCompleted = 0, date}: HabitDayProps)
{
    ...
    const completedPercentage = amount ? Math.round((completed / amount) * 100) : 0
    ...
    return (
        <Popover.Root>
            <Popover.Trigger 
                className={clsx("w-10 h-10 rounded-lg border-2 transition-colors focus:outline-none focus:ring-2 focus:ring-violet-900 focus:ring-offset-2 focus:ring-offset-background",
                {
